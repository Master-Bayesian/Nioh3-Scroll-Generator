...
try{
 ...
 check('Permanent edit waits for title-screen or closed confirmation',await p.getByRole('button',{name:'确认写入存档',exact:true}).isDisabled());await p.getByRole('checkbox',{name:'游戏已回到标题界面或已关闭',exact:true}).check();await p.getByRole('button',{name:'确认写入存档',exact:true}).click();await p.getByText('修改已写入存档。',{exact:true}).waitFor();check('Reviewed edit commits only to synthetic fixture',!original.equals(await readFile(fixture.path)));
 ...
 const backupRows=await p.locator('.backup-list tbody tr').count();await p.locator('.backup-list tbody input').last().check();await p.getByRole('button',{name:'恢复选中备份',exact:true}).click();await p.getByRole('button',{name:'确认恢复存档',exact:true}).waitFor();check('Backup restore waits for title-screen or closed confirmation',await p.getByRole('button',{name:'确认恢复存档',exact:true}).isDisabled());await p.getByRole('checkbox',{name:'游戏已回到标题界面或已关闭',exact:true}).check();await p.getByRole('button',{name:'确认恢复存档',exact:true}).click();await p.getByText('备份已恢复。',{exact:true}).waitFor();check('Backup restores the original encrypted fixture',original.equals(await readFile(fixture.path)));
