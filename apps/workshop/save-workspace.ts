...
const LAST_SAVE='nioh3-last-save';
const rememberedSave=()=>{try{return localStorage.getItem(LAST_SAVE)}catch{return null}};
saveSession?.subscribe(()=>{const id=saveSession!.getSnapshot().selected?.save_id;if(id)try{localStorage.setItem(LAST_SAVE,id)}catch{/* per-viewer convenience only */}});
let discovery:Promise<SaveReference[]>|null=null;
export function discoverSaves(refresh=false){if(refresh)discovery=null;if(!discovery)discovery=(async()=>{const result=await saveObserver!.run(()=>window.operations.execute({method:'save.discover',params:{}}));if(!result||!('saves' in result))throw Error('未能检测存档。');if(!saveSession!.getSnapshot().selected){const choice=result.saves.length===1?result.saves[0]:result.saves.find(s=>s.save_id===rememberedSave());if(choice)await saveSession!.select(choice)}return result.saves})().catch(error=>{discovery=null;throw error});return discovery}
