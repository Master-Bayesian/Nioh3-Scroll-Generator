param([Parameter(Mandatory)][int]$ProcessId,[Parameter(Mandatory)][string]$Target,[Parameter(Mandatory)][string]$Staged,[Parameter(Mandatory)][string]$ManifestHash)
...
foreach($folder in @($targetPath,$stagePath)){
    if(-not (Test-Path -LiteralPath (Join-Path $folder 'Nioh3ScrollEditorV2.exe')) -or -not (Test-Path -LiteralPath (Join-Path $folder 'build-manifest.json'))){throw 'Not a complete application package'}
}
