...
当前稳定版是基于 **Tauri 2** 重构的 **v0.7.2**。发布包已经包含应用所需的组件；唯一的系统依赖是 Microsoft Edge WebView2 Runtime，现代 Windows 通常已经自带。

[下载最新版本](https://github.com/Master-Bayesian/Nioh3-Scroll-Generator/releases/latest)
· [查看 v0.7.2 发布说明](packaging/release-notes.md)
...
受支持的结果既可以在游戏运行时直接添加，也可以在完全关闭游戏后写入存档。每次添加前，应用都会检查当前游戏和存档状态、创建并验证备份，然后记录本次操作。存档写入会在检测到游戏仍在运行时停止，避免退出游戏时用旧内存状态覆盖刚写入的文件。

v0.7.2 **不需要** Cheat Engine、Python、Node.js 或 Electron。兼容性与安全检查已经包含在应用内；遇到不受支持的游戏版本或存档状态时，操作会停止，而不会猜测性写入。
...
1. 从[最新版本页面](https://github.com/Master-Bayesian/Nioh3-Scroll-Generator/releases/latest)下载 `Nioh3Studio-0.7.2-win-x64-setup.exe`。
...
3. **添加：**选择游戏内实时添加，或关闭游戏后添加到存档，然后复核操作。
...
独脚踏鞴工作室不会止步于当前的绘卷功能。更多工具正在开发中；v0.7.2 里暂时不可用的“即将推出”入口是有意保留的。新工具会在用户流程和安全边界得到验证后再写入这里。
