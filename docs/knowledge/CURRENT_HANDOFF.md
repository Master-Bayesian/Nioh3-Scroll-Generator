...
**Release readiness and latest live acceptance (2026-09-21).** The tree now
carries version `0.8.0` in every synchronized file (`package.json`,
`package-lock.json`, `nioh3_scroll_editor/version.py`,
`apps/tauri/src-tauri/Cargo.toml`, `tauri.conf.json`, `apps/launcher/Cargo.toml`),
so the earlier "version deliberately still 0.7.5" statements describe the
historical internal backend-review candidate. The latest live acceptance (seed
`123456`, operation `0c9ac263-66fd-4619-bbfd-b10c2656cff2`) verified one native
add persisted on disk: 45 occupied records against 44 before, all 44 previous
records preserved under `defined()`, one new serial 2502886 at slot 30 (seed
123456, rarity 4), and the documented `0x18` flags transition `0x82 -> 0x80`.
Scope: read-only disk check - no native add in that run, no debugger, no restart -
so no save-reload, in-game re-read, or broader PC v2.02 write enablement is
claimed. The public latest is still `v0.7.5`, the v0.8.0 candidate commit is not
frozen, and publication awaits explicit owner authorization. Readiness:
...
that path previously read, and both versions share the one
`NativeLiveAddExecutor`. Version-selection gates report 164 `nioh3-runtime`, 60
`nioh3-protected` and 11 Python tests passed with both clippy runs clean; no
packaged-build or save-reload acceptance is claimed, and the earlier local
package `v080-local-rc-03174b9` predates this scope. The owner has authorized
packaging and the push/workflow step for this release; the exact candidate SHA
and tag are named at dispatch time.
...
A direct (save-path) install writes the installed scroll's `+0x18`..`+0x1B`
lifecycle word as the game's own insertion result, `0x06800082`
(`0x02800002 | 0x04000080`), instead of inheriting the donor template's flags, so
the new record is left unrevealed with the new-item marker. Implemented only at
the installation boundary in `nioh3_scroll_editor/savegame.py` and
`crates/nioh3-save/src/{codec,transform}.rs`; previews, materialization,
generated bytes, editor/delete/restore and live add are unchanged. Evidence:
`tests/test_post_insertion_state.py`, `nioh3-save` unit tests, and
`tests/migration/test_save_batch_install_parity.py` (Rust/Python saves remain
byte-identical, R3/R4/R5).

**Owner-reported game display acceptance (2026-09-21).** On the local RC source
`03174b9201958ebbab65fc4ede4054d5b9dba675` (outer EXE SHA-256
`2b8aff26787ac320f7c978634f9cac42d512ee31d92c7d4d759087ce0841b4b5`), the owner
reports that a scroll added through the save path displays normally: it is
unrevealed and shows the fixed-affix icon. This supersedes the earlier statement
that the in-game icon was unproven. The statement is an owner report on those
named bytes only; it does not add raw captures, an exact seed or rarity, repeat
or restart persistence, real-time native addition on game PC v2.02, broader
write enablement, or publication. The other native-live-add v2.02 gates remain
open. Details:
[the v0.8.0 record](../product/releases/v0.8.0.md).
...
## PC v2.02 live-add preview Pro handoff (2026-09-21)

Targeted bounded handoff: `D:\Nioh3_v080_deliverables\deliverables\v202-live-add-preview-pro-20260921\`
plus the same-named `.zip` (156,758 bytes, 36 files + `SHA256SUMS.txt`, ZIP
SHA-256
`555e3dda8e1a3ec248e432fd5a7231d4c1b8272beec21841c163e745f553e21b`). The
repository validator (`tools/validate_research_handoff.py`, run through
`tools/run_python_tests.ps1` with the resolved project Python) passed the
directory and the ZIP on 2026-09-21: `ok: true`, 36 directory hashes verified,
37 archived members verified.

It asks exactly two questions about the PC v2.02 live-add preview that stopped
at the builder-output review gate on 2026-09-21 (seed `226061463`, rarity 4,
`final_record`): (Q1) the builder flag word `0x00800002` versus the
v2.01-ported expectation `0x02800002` - whether bit 25 (`0x02000000`) is part
of the v2.02 builder output at all, and what version-correct normalization is;
and (Q2) safe preview-only rejection and receipt recovery, with separate
recommendations for the one historical unsettled receipt `178625a6` (it has no
durable pre-dispatch inventory fingerprint, and that absence cannot be assumed
away) and for future fail-closed, no-replay settlements.

Boundary: the preview executed the game's native builder and allocation path in
scratch memory under debugger control - no inventory insertion and no tool write
to the on-disk save occurred, and verified cleanup is recorded - but no
settlement, insertion, persistence, propagation, live acceptance, or product
readiness is claimed, and the receipt was left unsettled on purpose. PC v2.02
protected writes remain disabled and unapproved, the other native-live-add
v2.02 gates remain open, and this entry records no source change, push, tag, or
release. The package was built before this entry, so recording the link here
changes no package hash.
...
SHA-256
`932103512ddc3847a115088d1dbae0a7649fee12ea26aee52c5f28d92a0ae5cc`); the input
handoff it answered remains the `v202-live-add-preview-pro-20260921` package
(`555e3dda...`). The return holds `HOLD_UNRESOLVED_HISTORICAL_PREVIEW` for the
historical receipt, not an approval.

Static flags-context findings
(`D:\Nioh3_v080_deliverables\deliverables\v202-flags-context-20260921\SUMMARY.json`,
offline) close the bit-25 mechanism: the verified wrapper `0x228758C` writes bit
25 (`0x02000000`) **when effective identity `J` equals ambient `A`** - it calls
setter `0x1A7D7F4` (`{out+0x18, index 25, set}`) then helper `0x4273E8`
(`0x227FD96`/`0x227FD99`/`0x227FD9E`); **other later writes to that word are not
excluded**. The observed word `0x00800002` is exactly `bit1 | bit23`, consistent
with `J != A`.

Corrections to the current narrative:

- The observed builder/expectation mismatch is **not proven to be a version
  change**; it is consistent with the identity conditional above.
- The setter conditional `J == A` exists, and its semantics are proven
  statically.
- The candidate identity `0x1111222233334444` is a **synthetic fixture** (it
  matches the repo test constant); the actual ambient A was never observed, so no
  version-specific flag word is established.

Status: the future-preview settlement repair is **implemented** with bounded
offline validation only - `nioh3-runtime` tests report **150 passed / 0 failed**
(13 new), the Python runtime parity gates report 6 and 8 passed, and clippy plus
the example build are clean. Its formal real `run_dispatch` evidence runs the
shipped loop over a **scripted debug session, not a live OS/game session**, and
the bounded static recheck (`REVIEW.md` beside the report) **passed** - all three
scoped findings fixed, no remaining P0/P1/P2 and no blocker in that review.
Windows real debugger/game acceptance is still unperformed, so no acceptance is
claimed.
The historical receipt `178625a6` stays **untouched and blocked**; its local full
external before/after, index and thread-cleanup evidence remains **corroboration
only**, not a native durable inventory baseline. Source:
`D:\Nioh3_v080_deliverables\deliverables\v202-preview-settlement-repair\REPORT.md`.
Boundary: offline - no game call, no product flag constant change, no insertion,
no save write, no receipt change or settlement, no publication and no new RC.
Details: [the v0.8.0 record](../product/releases/v0.8.0.md).
...
same bytes, SHA-256 `da227b1a205d32bb53bb2095977e323f49d83e627733de38a8b99b84a3022fa`,
59,238 bytes - and the decision is a sidecar beside it,
